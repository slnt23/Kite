...
const finalTarget = computed(() =>
  redirectTarget.value || getDefaultRouteForUser({ role: selectedRole.value }),
)
...
const roleOptions = [
  { value: 'admin', label: '管理员', description: '登录后进入独立后台管理界面' },
  { value: 'user', label: '普通用户', description: '登录后进入前台个人管理界面' },
]
...
const submitLogin = () => {
  ...
  submitting.value = false
  secret.value = ''
  router.replace(finalTarget.value)
}
  ...
  <div class="login-screen">
    <section class="login-hero">
      ...
      <div class="login-hero__content">
        ...
        <p class="login-hero__summary">
          当前登录页支持管理员和普通用户两种角色，分别对应后台管理界面与前台个人管理界面。
        </p>
    ...
    <section class="login-stage glass-panel">
      <div class="login-stage__intro">
        ...
        <h2>选择身份与登录方式</h2>
        <p>管理员会进入隐藏前台导航的后台界面，普通用户则进入保留前台导航的个人管理页。</p>
      </div>
      ...
      <div v-if="!currentUser" class="login-stage__body">
        ...
        <aside class="login-card login-card--demo">
          ...
          <div class="login-card__copy">
            <h3>当前演示信息</h3>
            <p>根据所选角色切换示例账号，后续接入真实接口时可以直接沿用这套结构。</p>
            <ul class="login-demo-list">
              <li><strong>身份：</strong>{{ selectedRoleDemo.roleLabel }} / {{ selectedRoleDemo.displayName }}</li>
              <li><strong>密码：</strong>{{ selectedRoleDemo.password }}</li>
              <li><strong>验证码：</strong>{{ selectedRoleDemo.code }}</li>
            </ul>
          </div>
        </aside>
      </div>
      ...
      <div v-else class="login-state">
        <div class="login-state__panel">
          ...
          <p>当前身份为 {{ currentUser.roleLabel }}，你可以进入对应的管理界面，或者退出登录重新切换身份。</p>
          <div class="login-state__actions">
            <button class="login-form__submit" type="button" @click="router.push(getDefaultRouteForUser(currentUser))">
              进入管理界面
            </button>
