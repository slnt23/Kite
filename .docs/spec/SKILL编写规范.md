...
| 范围   | 路径                           | 说明                       |
| ------ | ------------------------------ | -------------------------- |
| 项目级 | `.trae/skills/<skill-name>/`   | 仅当前项目可用，优先级最高 |
| 用户级 | `~/.trae/skills/<skill-name>/` | 所有项目可用               |
| 内置   | 系统内置目录                   | 平台预装，不可修改         |
...
```yaml
# ✅ 清晰说明功能 + 明确触发条件
description: "Reviews Vue/TypeScript code for best practices, bugs, and security issues. Use when user asks for code review, before merging PRs, or when analyzing code quality."
...
# ✅ 包含领域关键词
description: "Kite project architecture guide for frontend module organization. Use when creating new business modules, refactoring code structure, or asking about project conventions."
```
...
```yaml
# 覆盖多种触发方式
description: 'Kite project Vue component generator. Use when creating new Vue components, scaffolding pages, adding UI elements, or when user asks to create a new component/page in the Kite project.'
```
...
description: 'Reviews Vue/TypeScript code for best practices, bugs, and security issues. Use when user asks for code review, before merging PRs, or when analyzing code quality.'
version: '1.0.0'
tags: ['code-review', 'quality', 'vue', 'typescript']
---

# Code Reviewer

对 Vue 3 + TypeScript 代码进行系统化审查，覆盖最佳实践、常见错误、安全漏洞和性能问题。
...
- 代码文件已存在且可读
- 了解项目架构规范（见 `.docs/spec/开发规范.md`）

## 审查清单

### 1. 架构规范

- [ ] 业务代码是否放在 `src/modules/<feature>/` 下
- [ ] 是否违反项目边界约束（不在 `core/` 放业务代码等）
- [ ] 页面是否只做展示与协调，业务逻辑是否在 composables/services/store 中
...
- 项目使用 TypeScript
- API 类型定义在 `src/modules/<feature>/types/` 中
- Python 3.8+ 可用（用于脚本）
...
扫描目标模块的 `types/` 目录，提取 DTO/VO 类型定义。
...
```bash
python scripts/generate-openapi.py --input <feature>/types --output <feature>/api-docs.yaml
```
````
...
生成的 OpenAPI 文档放在 `src/modules/<feature>/api-docs.yaml`。
