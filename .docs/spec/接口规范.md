...
本文档沉淀前端对接后端接口时需要遵守的规范，供后续联调和开发参考。接口事实以 `.docs/spec/frontend-api.md` 为准。

## 1. 接口对接规范

- 后端基础地址以 `.docs/spec/frontend-api.md` 为准，当前默认 `http://127.0.0.1:18080`。
...
- 接口文件统一放在 `src/modules/<domain>/api/`，按资源拆分。
- 类型统一放在 `src/modules/<domain>/types/`，通过模块 `index.ts` 导出；跨模块共享类型放 `src/shared/types/`。
- 常量统一放在 `src/modules/<domain>/constants/`，不允许把接口返回数据写进常量冒充真实数据。
- 登录、注册、验证码、重置密码等认证接口归属 `src/modules/user/api/`，与后端用户中心设计保持一致。
